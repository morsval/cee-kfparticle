...
void StKFParticleInterface::ResizeTrackPidVectors(const int nTracks)
{
  ...
  //=== the following part is added by q.hu on 2023.06.16
  fTrackdEdX.clear();
  fTrackdEdX.resize(nTracks, -1);
 
  fdEdxErrorFit.clear();
  fdEdxErrorFit.resize(nTracks, -1);
/*
  fPx.clear();
  fPx.resize(nTracks, -1);

  fPy.clear();
  fPy.resize(nTracks, -1);

  fPz.clear();
  fPz.resize(nTracks, -1);
*/
  fNHits.clear();
  fNHits.resize(nTracks, -1);
  
	fNHitsMax.clear();
  fNHitsMax.resize(nTracks, -1);

  fdca.clear();
  fdca.resize(nTracks, -1);

  fMass2.clear();
  fMass2.resize(nTracks, -1);

  fTofBeta.clear();
  fTofBeta.resize(nTracks, -1);
}
...
//   if(!IsGoodPV(primaryVertex)) return 0;
  ...
  for (Int_t iTrack = 0; iTrack < nGlobalTracks; iTrack++) 
  {
    ...
    TVector3 trackMomentum = gTrack->gMom();
    if (trackMomentum.Pt() < 0.05 || trackMomentum.Pt() > 10.0)
      continue;
    if (((double)gTrack->nHitsFit() / (double)gTrack->nHitsMax() < 0.52))
      continue;
    // if(gTrack->dEdxError() < 0.04 || gTrack->dEdxError() > 0.12) continue;//==it's included in the ProcessEvent function,2023.06.21
...
//       if(gTrack->hasSstHit()) nHftHitsInTrack++;
    ...
    //== the following part is added by q.hu on 2023.06.16
    if(isTofm2){fMass2[index]=m2tof;}
    else{fMass2[index]=-999;}

    const Double_t field = picoEvent->bField();//== added by q.hu on 2023.06.23
    StPicoPhysicalHelix helix = gTrack->helix(field);
    fdca[index] = fabs(helix.geometricSignedDistance(picoPV));

    fNHits[index] = gTrack->nHitsFit();//==added by q.hu  on 2023.06.23  
    fNHitsMax[index] = gTrack->nHitsMax();  
    fTrackdEdX[index] = gTrack->dEdx();//==added by q.hu  on 2023.06.23
