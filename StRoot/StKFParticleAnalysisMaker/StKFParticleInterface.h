...
class StKFParticleInterface: public TObject
{
 public:
  ...
  const float GetNHitsMax(const int trackId)  const { return fNHitsMax[trackId]; }
  const float GetdEdxErrorFit(const int trackId)  const { return fdEdxErrorFit[trackId]; }

  const float Getdca(const int trackId)  const { return fdca[trackId]; }
  //const float GetPx(const int trackId)  const { return fPx[trackId]; }
  //const float GetPy(const int trackId)  const { return fPy[trackId]; }
  //const float GetPz(const int trackId)  const { return fPz[trackId]; }
  const float GetMass2(const int trackId)  const { return fMass2[trackId]; }
  const float GetTofBeta(const int trackId)  const { return fTofBeta[trackId]; }
 ...
 private:
  ...
  std::vector<float> fdEdxErrorFit;
  std::vector<int> fNHits;
  std::vector<int> fNHitsMax;
  std::vector<float> fdca;
  //std::vector<float> fPx;
  //std::vector<float> fPy;
  //std::vector<float> fPz;
  std::vector<float> fMass2;
  std::vector<float> fTofBeta;
